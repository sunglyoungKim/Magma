...
class MagmaMultiModalProjector(nn.Module):
    def __init__(self, config):
        ...
        vision_tower = config.get('mm_vision_tower', 'segtokv1_xxlarge')
        vision_backbone_size = vision_backbone.replace('convnext', '')
        if '_' not in vision_tower:
            vision_tower = f'{vision_tower}_{vision_backbone_size}'
        ...
        # define two layer convnet
        self.segtok_version = int(mlp_gelu_match.group(2))

        if self.segtok_version == 1:
            dim_segtok_in = 3 * dim_vision[vision_tower.split('_')[-1]]
            dim_segtok_out = config['mm_hidden_size']
            self.proj_segtok = nn.Linear(dim_segtok_in, dim_segtok_out)
        elif self.segtok_version in [2, 3]:
            dim_segtok_in = dim_vision[vision_tower.split('_')[-1]]
            dim_segtok_out = config['mm_hidden_size']
            self.proj_segtok = nn.Linear(dim_segtok_in, dim_segtok_out)            
        elif self.segtok_version == 6:
            dim_segtok_in = 512 + dim_vision[vision_tower.split('_')[-1]]
            dim_segtok_out = 512 + dim_vision[vision_tower.split('_')[-1]]
            self.proj_segtok = nn.Sequential(
                nn.Conv2d(dim_segtok_in, dim_segtok_out, kernel_size=3, stride=2, padding=1, groups=dim_segtok_out, bias=False),
                nn.GELU(),
                nn.Conv2d(dim_segtok_out, dim_segtok_out, kernel_size=3, stride=2, padding=1, groups=dim_segtok_out, bias=False),
                nn.GELU(),
                nn.Conv2d(dim_segtok_out, dim_segtok_out, kernel_size=3, stride=2, padding=1, groups=dim_segtok_out, bias=False),                
            )
        elif self.segtok_version == 7:
            dim_segtok_in = 512 + 201
            dim_segtok_out = 512 + 201
            self.proj_segtok = nn.Sequential(
                nn.Conv2d(dim_segtok_in, dim_segtok_out, kernel_size=3, stride=2, padding=1, groups=dim_segtok_out, bias=False),
                nn.GELU(),
                nn.Conv2d(dim_segtok_out, dim_segtok_out, kernel_size=3, stride=2, padding=1, groups=dim_segtok_out, bias=False),
        ...
        elif self.segtok_version in [8, 9, 10]:
            dim_segtok_in = 512 + 3*dim_vision[vision_tower.split('_')[-1]]
            dim_segtok_out = 512 + 3*dim_vision[vision_tower.split('_')[-1]]
            self.proj_segtok = nn.Sequential(
                nn.Conv2d(dim_segtok_in, dim_segtok_out, kernel_size=3, stride=2, padding=1, groups=dim_segtok_out, bias=False),
                nn.GELU(),
                nn.Conv2d(dim_segtok_out, dim_segtok_out, kernel_size=3, stride=2, padding=1, groups=dim_segtok_out, bias=False),
    ...
    def forward(self, x):
        backbone_feature, segtok_feature = x
        if backbone_feature is not None and segtok_feature is not None:
            if self.segtok_version in [1, 2]:
                out_feature = self.proj_segtok(segtok_feature[-1])
                x = torch.cat([backbone_feature, out_feature], dim=1)
            elif self.segtok_version == 3:
                out_feature = self.proj_segtok(segtok_feature[-1])
                x = torch.cat([out_feature, backbone_feature], dim=1)                
            elif self.segtok_version == 6:
                out_feature = self.proj_segtok(segtok_feature[-1]).flatten(2).transpose(1, 2)
                backbone_feature = backbone_feature.flatten(2).transpose(1, 2)
                # x = self.proj(backbone_feature) + self.proj_segtok(out_feature)
                x = torch.cat([backbone_feature, out_feature], dim=2)
                # x = self.crossattn(backbone_feature, segtok_feature[-1])
            elif self.segtok_version == 7:
                out_feature = self.proj_segtok(segtok_feature[-1]).flatten(2).transpose(1, 2)
                backbone_feature = backbone_feature.flatten(2).transpose(1, 2)
                # x = self.proj(backbone_feature) + self.proj_segtok(out_feature)
                ...
                x = torch.cat([backbone_feature, out_feature], dim=2)
                # x = self.crossattn(backbone_feature, segtok_feature[-1])    
        elif backbone_feature is None and segtok_feature is not None:
            if self.segtok_version in [1, 2]:
                x = self.proj_segtok(segtok_feature[-1])
            elif self.segtok_version == 3:
                x = self.proj_segtok(segtok_feature[-1])
            elif self.segtok_version == 6:
                x = self.proj_segtok(segtok_feature[-1]).flatten(2).transpose(1, 2)
            elif self.segtok_version == 7:
                x = self.proj_segtok(segtok_feature[-1]).flatten(2).transpose(1, 2)
            elif self.segtok_version in [8, 9, 10]:
                x = self.proj_segtok(segtok_feature[-1]).flatten(2).transpose(1, 2)
        elif backbone_feature is not None and segtok_feature is None:                   
            x = backbone_feature                
        return self.proj(x)
